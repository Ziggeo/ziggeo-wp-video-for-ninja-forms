	...
	add_action('admin_menu', function() {
		if(function_exists('ziggeo_p_add_addon_submenu')) {
			ziggeo_p_add_addon_submenu(array(
				'page_title'    => 'Ziggeo Video for Ninja Forms',      //page title
				'menu_title'    => 'Ziggeo Video for Ninja Forms',      //menu title
				'capability'    => 'manage_options',                    //min capability to view
				'slug'          => 'ziggeoninjaforms',                  //menu slug
				'callback'      => 'ziggeoninjaforms_show_form')        //function
			);
		}
		else {
			add_action( 'admin_notices', function() {
				...
				<div class="error notice">
					<p><?php _e( 'Please install <a href="https://wordpress.org/plugins/ziggeo/">Ziggeo plugin</a>. It is required for this plugin (Videowalls for Ziggeo) to work properly!', 'ziggeoninjaforms' ); ?></p>
				</div>
				<?php
			});
		}
	...
	function ziggeoninjaforms_show_form() {
		...
		function ziggeoninjaforms_d_hooks() {
			...
			function ziggeoninjaforms_g_captured_content() {
				...
				<label for="ziggeoninjaforms_capture_content"><?php _e('Depending on your choice here you will change what is captured once the video is recorded', 'ziggeoninjaforms'); ?></label>
				<?php
			}
